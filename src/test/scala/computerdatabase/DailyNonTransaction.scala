...
class DailyNonTransaction extends Simulation{
  ...
  val loginFeeder8000 = csv("data/EMPLogin3500.csv").random
  ...
  var loginClaimFeeder1 = csv("data/ESSEMPLogin-Claim100.csv").random
  ...
  setUp(
    ...
    ApplyLeaveSaveDraft.inject(rampUsers(EMPNo).during(scriptDuration.minutes)), //7
    ...
    MyWorkflowViewLeaveApplication.inject(rampUsers(SUPNo).during(scriptDuration.minutes)), //10
    ...
    ExpensesDetailsforClaimListing.inject(rampUsers(HRNo).during(scriptDuration.minutes)) //9
